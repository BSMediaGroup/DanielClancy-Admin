...
const manifest = JSON.parse(await readFile(new URL("../assets/data/location-cover-images.json", import.meta.url), "utf8"));
const siblingManifest = JSON.parse(await readFile(resolve(workspaceRoot, "StreamSuites-Dashboard", "docs/shared/data/location-cover-images.json"), "utf8"));
...
function localAssetPath(imagePath) {
  return join(repoRoot, imagePath.replace(/^\//, ""));
}
...
test("cover helpers resolve required local raster covers", () => {
  assert.match(getLocationCoverImage({ city: "Portland", region: "Oregon", countryCode: "US" }).imagePath, /city-us-oregon-portland\.webp$/);
  ...
  assert.match(getLocationCoverImage({ city: "Rio de Janeiro", region: "Rio de Janeiro", countryCode: "BR" }).imagePath, /city-br-rio-de-janeiro-rio-de-janeiro\.webp$/);
...
test("popup and sidebar cover UI has bounded frames and MapLibre popup overrides", () => {
  assert.match(mapModule, /class="analytics-map-popup-cover"/);
  assert.match(app, /class="analytics-map-selected-cover"/);
  assert.match(css, /\.analytics-map-popup \.maplibregl-popup-content\s*\{[\s\S]*max-width:\s*min\(420px,\s*calc\(100vw - 32px\)\)\s*!important/);
  assert.match(css, /\.analytics-map-popup \.maplibregl-popup-close-button\s*\{[\s\S]*z-index:\s*3[\s\S]*color:\s*#f7fbff/);
  assert.match(css, /\.analytics-map-popup-cover\s*\{[\s\S]*aspect-ratio:\s*16 \/ 9/);
  assert.match(css, /\.analytics-map-popup-cover img\s*\{[\s\S]*object-fit:\s*cover[\s\S]*object-position:\s*center/);
  assert.match(css, /\.analytics-map-selected-cover\s*\{[\s\S]*aspect-ratio:\s*16 \/ 9/);
  assert.match(css, /\.analytics-map-selected-cover img\s*\{[\s\S]*object-fit:\s*cover[\s\S]*object-position:\s*center/);
});
