...
async function writeCollection(context, collection, session) {
  ...
  logAlertFailure(
    ...
    await postDanielClancyAlert(context, {
      ...
      pagePath: `#/${collection}`,
      pageUrl: `https://admin.danielclancy.net/#/${collection}`,
      displayName: session?.display_name || session?.email || "",
      userEmail: session?.email || "",
      userType: session?.account_type || "admin",
      accountType: session?.account_type || "admin",
      ...
      payload: {
        ...
        user_type: session?.account_type || "admin",
        account_type: session?.account_type || "admin",
        updatedAt,
      },
    }),
  );
