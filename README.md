...
Optional StreamSuites live analytics read:

- `STREAMSUITES_ANALYTICS_URL` - server-side URL for the StreamSuites runtime/API DanielClancy-only analytics feed, normally `https://api.streamsuites.app/api/analytics/danielclancy`.
- `DANIELCLANCY_ANALYTICS_READ_SECRET` - generated server-only read secret sent by DanielClancy-Admin to the StreamSuites read endpoint. Do not expose this in browser code.
...
When `STREAMSUITES_ANALYTICS_URL` is configured, `GET /api/admin/analytics` first reads the StreamSuites DanielClancy-only live feed server-side, forwarding the selected `5m|15m|1h|24h` window and `DANIELCLANCY_ANALYTICS_READ_SECRET` when present. StreamSuites live rows have source `streamsuites_live` and take priority over local Admin KV and Cloudflare GraphQL rows for live location markers/tables. If StreamSuites is unavailable, Admin falls back to source-tagged Admin KV rows and then Cloudflare GraphQL aggregates without treating stale/sample local rows as live.
...
The page-visit endpoint stores non-secret metadata only: `source`, `live`, `eventId`, `recordedAt`, optional safe session id, surface, page path/url/title, referrer host, country, region, city, timezone, colo, browser, device, platform, and safe admin/authenticated flags. It does not store raw client IP addresses. Page-visit reads are filtered by the selected analytics window; live rows outside the selected window, rows missing event timestamps, sample/demo/fallback rows, and untagged legacy rows are not mixed into the active map/table counts.
...
City-level location detail comes from page-visit KV rows enriched with Cloudflare request geo metadata such as `request.cf.city`; the public sender must forward that sanitized geo because the Admin ingest request is server-to-server and cannot recover the original visitor `request.cf` by itself. Cloudflare GraphQL can provide selected aggregate sections but may not provide city rows for this account/dataset, so Admin analytics requires events ingested into Admin KV or an event-only StreamSuites mirror to show city-level live locations. Referrer host is returned from page-visit KV when available; if the current Cloudflare dataset does not expose a safe referrer dimension, Cloudflare referrers are reported as unavailable with the note “Referrer host is unavailable from the current Cloudflare dataset.” If only region/country rows are available, the API and UI mark those rows with `precision: "region"` or `precision: "country"` and do not pretend country rows are city rows. When zero page-visit events exist, the API/UI says “No page-visit events have been captured yet.”

The Analytics map/location panel uses a real interactive MapLibre GL map, modelled on the StreamSuites-Dashboard MapLibre analytics pattern. Runtime MapLibre assets are vendored locally under `assets/vendor/maplibre-gl/`, while the dark basemap uses CARTO `dark_all` raster tiles with the required OpenStreetMap/CARTO attribution shown by MapLibre. Map behavior lives in the isolated `assets/js/analytics-map.js` module, with local city/country coordinate data split into `assets/js/geo-coordinate-lookup.js`, documented in `assets/data/geo-coordinate-lookup.json`, and paired with local real raster cover metadata in `assets/data/location-cover-images.json`; `assets/js/admin-app.js` renders the route shell/tables and passes selected-window live rows to the module. The cover pack stores local WebP files under `assets/analytics/location-covers/`, records Wikimedia Commons/Openverse sourcing plus attribution/license metadata, and reserves the default raster fallback only for unknown future locations or compact broken-image recovery. The map supports pan/zoom, expanded inline display, fullscreen lightbox display, the `analytics-live-locations` GeoJSON source, centered `analytics-location-halo` and `analytics-location-dot` circle layers, a transparent `analytics-location-hitbox` popup layer, selected-marker ring behavior, attribution, route/layout resize handling, and a visible empty overlay. Inline and fullscreen views share layer visibility state for `Location dots` and `Request glow`, so toggles affect MapLibre layer visibility without removing source data. Live markers are generated only from source-tagged, timestamped, selected-window rows (`streamsuites_live`, `page_visit_kv`, `cloudflare_graphql`, or `streamsuites_event_mirror`); sample/fallback/demo/mock/test rows and stale untagged legacy rows stay out of the map. Marker rows aggregate to one plotted GeoJSON feature per stable project/source/location/coordinate group, so repeated events for the same city do not render duplicate marker chains or DOM marker child dots. Marker dot radius represents sessions when session ids or upstream session counts are available; the larger centered halo circle represents requests/events. If session data is unavailable, sessions render as `n/a` rather than being invented. The fullscreen lightbox reuses the same MapLibre feature pipeline and includes selected window, total rows, sessions, city and country fallback marker counts, unmapped rows, last live event, selected-location card with local cover image, collapsible mapped/unmapped sidebar, source breakdown, precision legend, and marker legend.

Known city coordinates are plotted only from trusted explicit longitude/latitude fields, safely detected/corrected coordinate arrays, or exact city/region/country matches in the built-in lookup. Maseru, Maseru District, LS and the Lesotho country centroid are included in local lookup coverage. If a real city row lacks a verified city coordinate but has a valid ISO country code or supported country alias, the map plots one verified country centroid fallback marker with `coordinateSource: "country_centroid"` and `plottedPrecision: "country_fallback"` rather than dropping the row. Country fallback markers are visually distinct, popups say “Country fallback location,” and the Location Breakdown table keeps the original city/region text while showing `city`, `country fallback`, or `unmapped` as map precision. GeoJSON geometry is emitted only as `[longitude, latitude]`, and Portland/Los Angeles guardrails reject positive East Hemisphere event coordinates or coordinates outside their west-coast ranges before falling back to verified lookup data. Unmapped rows now mean no valid event coordinate, no city lookup, and no usable country centroid fallback. Unknown city coordinates are not invented, and no external coordinate service is called at runtime.

Country values render with the local `flag-icons` SVG set copied into `assets/icons/flags/`. In location tables, flags appear only in the Country column; City, Region, Precision, and Source remain text/badges only. Map layer popups/tooltips, country chips, and country summary labels retain country flags. Unknown or unsupported country codes use the local `_fallback.svg` globe icon; no remote flag CDN, base64 flag, or emoji flag fallback is used as the primary implementation.

The Analytics page shows `adminApiConnected`, `streamSuitesAnalyticsConfigured`, `streamSuitesAnalyticsConnected`, `kvConnected`, `cloudflareGraphqlConnected`, `analyticsIngestConfigured`, `sourceBreakdown`, `liveRowsCount`, `staleRowsCount`, `sampleRowsCount`, `lastLiveEventAt`, `Last refreshed`, `Last live page-visit event`, `Last Cloudflare GraphQL query`, grouped partial/unavailable query status, and a freshness state of `live_recent`, `live_stale`, `no_live_events`, `sample_only`, or `cloudflare_partial`. Raw GraphQL range/schema errors are kept out of the main visible notes. The Refresh analytics button performs a manual periodic refresh for the selected window; the UI does not claim realtime behavior. The admin-only purge action removes sample/fallback/demo/mock/test rows and untagged legacy rows while keeping source-tagged live rows.
...
Projects are built from the protected public Projects baseline plus safe `cms:projects` KV overlay when available. Watch media entries are read from `cms:media`, filtered through the scaffold/demo/sample detector, sanitized into `collections.watchMedia`, sorted newest-first by `sortDate`, `publishedAt`, `enteredAt`, `createdAt`, then `updatedAt`, and filtered to visible public-safe fields only. Authenticated Media reads and the visible `Purge scaffold rows` action remove rejected rows such as `latest-youtube-release-scaffold`, `scheduled-livestream-scaffold`, and `archived-replay-scaffold` from `DC_ADMIN_KV`; saves filter them again so they cannot be re-added through import or old localStorage. Empty or unavailable CMS reads show truthful empty/auth/config states instead of seeding media rows. Rumble video rows can include safe `https://rumble.com/embed/...` iframe URLs for public hero playback; Rumble short rows are exported as portrait gallery-only links with `heroEmbeddable: false`. Product storefront overrides are read from `cms:products`, sanitized, and filtered so private/internal rows are not exposed. The export `meta` includes `watchMediaCount`, `manualMediaCount`, `youtubeCount`, `scaffoldMediaPurgedCount`, `productOverrideCount`, and `bannerRegistryCount`, and publish response counts include `watchMedia`, `productOverrides`, and `banners` for public verification. Companies, Platforms, and Positions use the same `registry-overlay.v3` reconciliation layer as the admin CMS endpoints. Client-only organizations remain excluded from public Companies; source client/provenance labels can remain on project rows where they are public metadata. If `DC_ADMIN_KV` is unavailable or a collection read fails, the endpoint returns reconciled baseline data with warnings instead of failing the public website.
...
3. Media reads and saves use real `cms:media` data only. The table is newest-first by `sortDate`, `publishedAt`, `enteredAt`, `createdAt`, then `updatedAt`, with visible manual, hidden/draft, public revision, auto-publish, and purged scaffold counts shown in `#/media`.
...
6. Media saves to `cms:media` auto-publish sanitized `collections.watchMedia` so manual Rumble videos/shorts can appear on public `/watch` after refresh; edited `publishedAt` and `sortDate` are persisted as ISO strings.
7. For Watch Media verification, compare the Admin Media public revision/updated timestamp against `/api/public/site-data`, confirm `meta.manualMediaCount`, confirm the saved `watchMedia` row/date exists, then refresh public `/watch` and verify the newest embeddable item is the hero while Rumble shorts remain gallery-only.
8. The public site should fetch `https://admin.danielclancy.net/api/public/site-data` when `VITE_ADMIN_PUBLIC_SITE_DATA_URL` is configured in the public Cloudflare Pages project. Public `/watch` requests at least 12 YouTube feed items and merges them with sanitized Admin manual media without inventing filler rows.
9. Verify `GET /api/merch/products` on the public site reports the same `overrideRevision` and exposes product `banners` for assigned/enabled banner rows.
10. Refresh the public site. Redeploy Public only when the env var changed, the committed fallback snapshot changed, rendering code changed, public assets were added, or the public watch renderer changed.
...
│   ├── analytics-map-coordinate-fallback.test.mjs
│   ├── analytics-map-flags.test.mjs
│   ├── analytics-map-fullscreen-polish.test.mjs
│   ├── location-cover-images.test.mjs
│   ├── analytics-map-marker-lifecycle.test.mjs
│   ├── analytics-map-rebuild.test.mjs
...
- Media CMS for `/watch` with admin API/KV hydration when `DC_ADMIN_KV` is configured, truthful unavailable/empty states when live CMS is missing, newest-first table editing, Add Rumble Video/Short workflow, edit/detail modal, Rumble metadata resolution, manual title/description/thumbnail/date/visibility overrides, R2 thumbnail upload through `DC_ADMIN_ASSETS_R2`, visible manual/hidden draft/public revision/purged scaffold diagnostics, bulk actions, clear-empty, protected scaffold purge, and JSON copy/import controls. It does not render scaffold/demo/sample/placeholder media rows.
- Media CMS saves manual watch entries to `cms:media`, rejects or purges scaffold rows before persistence/export, and auto-publishes sanitized `collections.watchMedia` for DanielClancy.net. It does not execute or store raw Rumble scripts, does not mutate YouTube autofetch state, and does not connect to StreamSuites.
