...
- Analytics page hydrates Cloudflare GraphQL and page-visit KV readiness from `/api/admin/analytics`; missing/failed Cloudflare config is reported clearly, API/KV/ingest/GraphQL status is separated, city precision is labelled per row, and the location section uses a real dark MapLibre GL map with an aggregated GeoJSON source, halo/dot circle layers, and popups. Empty live analytics shows “No live page-visit location events captured yet.” over the real basemap and no fake sample markers.
